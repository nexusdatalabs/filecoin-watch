---
layout: "../layouts/Layout.astro"
---

import csvText from "../data/daily_metrics.csv?raw";
import LineChart from "../components/LineChart.astro";

export const latestEntries = (() => {
  const lines = csvText.trim().split(/\r?\n/);
  if (lines.length < 2) return [];
  const headers = lines.shift().split(",");
  const rows = lines.map((line) => line.split(","));
  const dateIdx = headers.indexOf("date");
  const latestRow = rows.reduce((best, cur) => {
    if (!best) return cur;
    // ISO date strings compare lexicographically
    return cur[dateIdx] > best[dateIdx] ? cur : best;
  }, null);
  return headers.map((h, i) => [h, latestRow?.[i] ?? ""]);
})();

export const series = (() => {
  const lines = csvText.trim().split(/\r?\n/);
  if (lines.length < 2) return { x: [], y: [], label: "" };
  const headers = lines.shift().split(",");
  const dateIdx = headers.indexOf("date");
  const metric = "providers_with_power";
  const metricIdx = headers.indexOf(metric);
  if (metricIdx === -1 || dateIdx === -1) return { x: [], y: [], label: metric };

  const rows = lines
    .map((line) => line.split(","))
    .filter((cols) => cols.length > metricIdx && cols[metricIdx] !== "");

  rows.sort((a, b) => (a[dateIdx] < b[dateIdx] ? -1 : a[dateIdx] > b[dateIdx] ? 1 : 0));
  const last = rows.slice(-180);

  const x = last.map((r) => r[dateIdx]);
  const y = last.map((r) => Number(r[metricIdx] || 0));
  return { x, y, label: "Providers with power" };
})();

<header class="masthead">
  <div class="logo">
    <a href={import.meta.env.BASE_URL} aria-label="Filecoin Watch home">
      <img src={import.meta.env.BASE_URL + 'favicon.svg'} alt="Filecoin Watch" width="56" height="56" decoding="async" loading="eager" fetchpriority="high" />
    </a>

    <nav class="navbar" aria-label="Primary">
      <ul>
        <li><a href="#about">About</a></li>
        <li><a href="#metrics">Metrics</a></li>
        <li><a href="#status">Status</a></li>
      </ul>
    </nav>

    <h1>Filecoin Watch</h1>
    <div class="subhead">
      An operational view into the Filecoin network
    </div>
  </div>
</header>

## About

Filecoin Watch is a tiny site that creates essential Filecoin reports across entities like Clients, Providers, Onramps, or Allocators using data from the Filecoin Data Portal.

## Metrics

<ul>
  {
    latestEntries.map(([key, value]) => (<li><strong>{key}:</strong> {value}</li>))
  }
</ul>

## Charts

<LineChart x={series.x} y={series.y} label={series.label} />

## Status

Super Alpha. Data wiring and charts coming soon.
